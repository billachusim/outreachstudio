...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clientProposal && !dryRun) {
      // Confirm phase — reuse the cached proposal, no AI call
      proposal = clientProposal;
    } else {
      // Dry-run OR first-time call → AI proposes
...
Propose the launch.`;
      ...
      if (aiResp.status === 402) return json(402, { error: "AI credits exhausted — top up in workspace settings" });
      if (!aiResp.ok) {
        const txt = await aiResp.text();
        return json(500, { error: `AI error: ${txt.slice(0, 200)}` });
      }

      const aiData = await aiResp.json();
      const call = aiData.choices?.[0]?.message?.tool_calls?.[0];
      if (!call) return json(422, { error: "AI returned no proposal" });

      try {
        proposal = JSON.parse(call.function.arguments);
      } catch {
        return json(500, { error: "Failed to parse AI proposal" });
      }

      if (!proposal.matchedOfferingId && !proposal.newOffering) {
        return json(422, { error: "AI could not match or invent a sensible offering for this story." });
      }
    }
    ...
    if (!offeringId && proposal.newOffering) {
      ...
      const { data: newOff, error: offErr } = await supabase
        ...
        .select("id")
        .single();
      if (offErr) return json(500, { error: `Offering insert failed: ${offErr.message}` });
    ...
    const { data: campaign, error: campErr } = await supabase
      ...
      .select("id")
      .single();
    if (campErr) return json(500, { error: `Campaign insert failed: ${campErr.message}` });
    ...
    const { data: run, error: runErr } = await supabase
      ...
      .select("id")
      .single();
    if (runErr) return json(500, { error: `Run insert failed: ${runErr.message}` });

    // Mark intel acted + link
    await supabase
      .from("intel_items")
    ...
    await supabase.from("run_events").insert({
      ...
      run_id: run.id,
      campaign_id: campaign.id,
      kind: "launched_from_intel",
      level: "info",
      message: `Spawned from intel: ${intel.title.slice(0, 100)}`,
    });
