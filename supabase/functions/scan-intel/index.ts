...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Scrape each source once (shared across all users)
    ...
    if (scraped.length === 0) return json(200, { ok: true, articles: 0, inserted: 0 });

    // 2. For each user with offerings, score articles and insert relevant ones
    ...
    for (const userId of userIds) {
      ...
      // Skip articles already stored for this user
      ...
      // Ask AI to score relevance + match offerings, in one batch call
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an outreach intel analyst. Given a creator's offerings + identity, and a list of news articles, score each article 0-100 for outreach value (would this be a good trigger to pitch a relevant offering to the company/people in the article?). Match relevant offering IDs. Return STRICT JSON only.",
            },
            {
              role: "user",
              content:
      ...
      const rows = scores
        .map((s: any) => {
          ...
          return {
            ...
            title: art.title,
            url: art.url,
            summary: art.summary ?? s.reason ?? null,
    ...
    return json(200, { ok: true, articles: scraped.length, inserted: totalInserted });
  } catch (e) {
    console.error("scan-intel error", e);
